        ...
        <div class="mb-3">
            ...
            <a href="{{url('ticket/print/'.$tickets->id)}}" target="_blank" class="btn btn-default btn-tool btn-sm">
                ...
                <div class="dropdown-menu">
                    ...
                    <?php if ( $tickets_approval->status != 7 && $tickets_approval->status != 2 && $tickets_approval->status != 3 && $group->can_edit_ticket == 1) { ?>
                    <a href="#" id="blocked" class="dropdown-item"><i class="fas fa-ban" style="color:#dd4b39;"> </i> {!! Lang::get('lang.blocked') !!}</a>
                    <?php } ?>
...
<script type="text/javascript">
 ...
 $('#approval_close').on('click', function(e) {
    ...
    type: "POST",
            ...
            // Set ticket to Blocked (status 9)
            $('#blocked').on('click', function(e) {
                e.preventDefault();
                $.ajax({
                    type: "POST",
                    url: "../ticket/blocked/{{$tickets->id}}",
                    data: { _token: "{{ csrf_token() }}" },
                    beforeSend: function() {
                        $("#hide2").hide();
                        $("#show2").show();
                    ...
                    success: function(response) {
                        ...
                        $("#hide2").show();
                        $("#show2").hide();
                        var message = "{!! Lang::get('lang.blocked') !!}";
                        $("#alert10").css('display','block');
                        $('#message-success0').html(message);
                        setInterval(function(){ $("#alert10").css('display','none'); }, 4000);
                        location.reload();
